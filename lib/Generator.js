...
module.exports = class Generator {
  ...
  async loadConfigModule() {
    ...
    for (const configKey of modules) {
      try {
        // eslint-disable-next-line
        await require('./configs/' + configKey + '.js')({
  ...
  async generatePages() {
    ...
    for (const page of this.pageList) {
      ...
      const templatePath = container && container !== defaultContainer ? path.resolve(this.rootDir, '.template', container) : path.resolve(__dirname, `./template/${defaultContainer}.ejs`);
    ...
    this.copyFiles.forEach(({ source, targetDir, type }) => {
      if (type === 'template') {
        ...
        try {
          ...
          if (this.useLocalBlocks && fse.existsSync(path.join(targetDir, 'style.d.ts'))) {
