...
const addLazyImport = (node, identifier, importPath, index) => {
  const { body } = node;
  console.log(identifier, importPath);
  const newVariableDeclaration = t.variableDeclaration(
    'const',
    [
      t.variableDeclarator(
        ...
        t.callExpression(
          t.identifier('lazy'),
          [
            t.arrowFunctionExpression([], t.callExpression(t.identifier('import'), [t.stringLiteral(importPath)])),
          ],
        ),
      ),
    ],
  );
  console.log('newVariableDeclaration', newVariableDeclaration, index);
  body.splice(index, 0, newVariableDeclaration);
};
