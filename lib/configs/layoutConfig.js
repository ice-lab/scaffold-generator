        ...
        JSXElement(nodePath) {
          ...
          if (
            ...
            SHELL_ATTRIBUTE.forEach((attribute) => {
              const value = layoutConfig[attribute.name] || attribute.defaultValue;
              const nodeValue = attribute.method === 'stringLiteral' ? t.stringLiteral(value) : t.jsxExpressionContainer(t[attribute.method](value));
              const attrNode = t.jsxAttribute(t.jsxIdentifier(attribute.name), nodeValue);
              node.openingElement.attributes.push(attrNode);
            });
          }
          // update Shell children node
